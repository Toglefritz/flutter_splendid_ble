...
All Bluetooth functionality provided by this plugin goes through the `FlutterBlePlugin` class. So,
wherever you need to conduct Bluetooth operations, you will need an instance of `FlutterBlePlugin`.
...
You could simply instantiate `FlutterBlePlugin` in each Dart class where it is needed or, depending
upon your needs and the architecture of your application, you could create a centralized service
where the `FlutterBlePlugin` instance is created once and referenced from everywhere else in your
...
class BluetoothManager {
  /// FlutterBlePlugin instance as discussed above.
  final FlutterBlePlugin _ble = FlutterBlePlugin();
...
When subscribing to BLE characteristic notifications or indications using the FlutterBlePlugin, the
...
class BLECharacteristicListener {
  ...
  /// Constructor accepts a FlutterBlePlugin instance and a BLECharacteristic instance.
...
actions or configure settings on the peripheral. The 'FlutterBlePlugin' provides a 'writeValue'
method
