...
extension ScanFilterListExtensions on List<ScanFilter>? {
  ...
  ///
  /// The main reason this logic is in an extension is to allow the same filtering logic to be applied in both the real
  /// `CentralMethodChannel` and the fake `FakeCentralMethodChannel`. This way, the same filtering logic can be used in
  ...
  /// Flutter side. This makes filtering more robust and also allows for more sophisticated filtering mechanisms to be
  /// applied without adding custom filtering mechanisms for each platform.
  bool deviceMatchesFilters(BleDevice device) {
    ...
    for (final ScanFilter filter in this!) {
      ...
      final bool matchesName =
          filter.deviceName == null || filter.deviceName == device.name;
      ...
      final bool matchesService = filter.serviceUuids == null ||
          device.advertisedServiceUuids.map((uuid) => uuid.toLowerCase()).any(
                (String uuid) => filter.serviceUuids!
                    .map((f) => f.toLowerCase())
                    .contains(uuid),
              );
      ...
      if (filter.manufacturerData != null && data != null) {
        ...
        final Map<int, List<int>> deviceMap = {
          (data.manufacturerId[0] | (data.manufacturerId[1] << 8)):
              data.payload,
        };
        ...
        if (filter.manufacturerData!.length == deviceMap.length &&
            filter.manufacturerData!.keys.every(
              (int key) =>
                  deviceMap.containsKey(key) &&
                  _listEquals(filter.manufacturerData![key]!, deviceMap[key]!),
            )) {
        ...
        matchesManufacturerData =
            filter.manufacturerData == null || filter.manufacturerData!.isEmpty;
      }
